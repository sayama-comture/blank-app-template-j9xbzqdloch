...
from app.routes.schemas.bot import type_shared_scope, type_sync_status, BotMetaOutputWithOwnerId #新規追加
from app.utils import get_current_time
from boto3.dynamodb.conditions import Attr, Key
from botocore.exceptions import ClientError

from app.user import User # 新規追加
...
# 新規追加 -----
def change_bot_owner(bot_id:str, user_id: str):
    """
    ボットの所有者を変更する関数
    
    引数:
        bot_id (str): 変更対象のボットID
        user_id (str): 新しい所有者のユーザーID

    処理概要:
        1. 指定されたボットIDでボット情報を取得
        2. 現在の所有者と新しい所有者が同じ場合は処理をスキップ
        3. 新しい所有者でボット情報を作成・登録
        4. 元の所有者のボット情報を削除
    
    例外処理:
        - ClientError: DynamoDBアクセス時のエラー
        - BotUpdateError: ボット更新時のエラー
    """
    logger.info("start change_bot_owner")
    logger.info(f"ボットID: {bot_id}, 新所有者: {user_id}")
    ...
    try:
        ...
        logger.error(f"DynamoDBクエリ中にエラーが発生しました。: {e}")
        raise
    
    # ボットIDは一意なので配列の一つ目を取得
    item = response["Items"][0]
    previous_owner_id = item["PK"]
    logger.info(f"ボット情報: {item}")
    
    # 変更前と変更後のユーザーが同じ場合、処理をスキップ
    ...
    items = {
        ...
        "AllowedCognitoGroups": item["AllowedCognitoGroups"],
        "AllowedCognitoUsers": item["AllowedCognitoUsers"],
        ...
        "ApiPublishmentStackName": item["ApiPublishmentStackName"],
        "ApiPublishedDatetime": item["ApiPublishedDatetime"],
        "ApiPublishCodeBuildId": item["ApiPublishCodeBuildId"],
        "DisplayRetrievedChunks": item["DisplayRetrievedChunks"],
        "ConversationQuickStarters": item["ConversationQuickStarters"],
        "ActiveModels": item["ActiveModels"],
        "UsageStats": item["UsageStats"]
    }

    # オプション属性を追加
    if item.get("LastUsedTime"):
        items["LastUsedTime"] = item["LastUsedTime"]
    if item.get("SharedScope"):
        items["SharedScope"] = item["SharedScope"]
    if item.get("IsStarred"):
        items["IsStarred"] = item["IsStarred"]
    if item.get("BedrockKnowledgeBase"):
        items["BedrockKnowledgeBase"] = item["BedrockKnowledgeBase"]
    if item.get("GuardrailsParams"):
        items["GuardrailsParams"] = item["GuardrailsParams"]

    # 変更後の項目を登録
    try:
        response = table.put_item(Item=items)
        logger.info(f"変更後の項目を登録しました: {items}")
    except ClientError as e:
        logger.error(f"Failed to change bot owner: {e}")
        raise BotUpdateError(f"Failed to change bot owner: {e}")

    # 変更前の項目を削除
    try:
        response = table.delete_item(
            Key={"PK": previous_owner_id, "SK": compose_sk(bot_id, "bot")},
        )
        logger.info(f"変更前の項目を削除しました: ユーザーID: {previous_owner_id}, ボットID: {bot_id}")
    except ClientError as e:
        logger.error(f"Failed to delete bot: {e}")
        raise BotUpdateError(f"Failed to delete bot: {e}")

    logger.info("end change_bot_owner")
    return
# 新規追加 -----
...
# 新規追加 ---
def find_all_shared_bots(user: User, limit: int | None = None) -> list[BotMetaOutputWithOwnerId]:
    """
    共有されたボット情報を取得する関数
    ...
    戻り値:
        List[BotMetaOutputWithOwnerId]: 共有されたボットのメタデータリスト
    
    Raises:
        Exception: DynamoDBクエリエラー、データ変換エラー等
    
    処理概要:
        1. 全体共有（SharedScope="all"）のボットを取得
        2. 部分共有（SharedScope="partial"）のボットから、ユーザーに共有されているものを取得
        3. 所有しているボットを取得
        4. 取得したボット情報をBotMetaOutputWithOwnerIdリストとして返却
    """
    logger.info("start find_all_shared_bots")
    logger.info(f"指定ユーザーの所有ボットと共有されたボットを取得します user: {user}")

    # DynamoDBテーブルクライアントを取得
    table = get_bot_table_client()
    bot_metas = []
    ...
    while query_count < MAX_QUERY_COUNT:
        query_count += 1
        logger.info(f"クエリ回数: {query_count}")
        try:
            response = table.query(**query_params)
        except ClientError as e:
            logger.error(f"DynamoDBクエリ中にエラーが発生しました。: {e}")
            raise
        bots.extend(response["Items"])
        if "LastEvaluatedKey" not in response:
            break
        query_params["ExclusiveStartKey"] = response["LastEvaluatedKey"]

    logger.info(f"全体共有されているボットの一覧: {bots}")
    ...
    for item in bots:
        ...
        # ボット情報をBotMetaOutputに変換
        bot_meta = BotMeta.from_dynamo_item(item, owned=False, is_origin_accessible=True)
        bot_meta_output = bot_meta.to_output()
        
        # オーナーID付きのBotMetaOutputWithOwnerIdに変換
        bot_meta_output_with_owner_id = BotMetaOutputWithOwnerId(**bot_meta_output.model_dump(), owner_user_id=item["PK"])

        # 戻り値に追加
        bot_metas.append(bot_meta_output_with_owner_id)
    ...
    logger.info(f"部分共有されているボットの一覧: {bots}")
    ...
    for item in bots:
        ...
        # 指定ユーザーに共有されているかチェック
        bot = find_bot_by_id(item["BotId"])
        if bot.is_shared_to_user(user):
            # ボット情報をBotMetaOutputに変換
            bot_meta = BotMeta.from_dynamo_item(item, owned=False, is_origin_accessible=True)
            bot_meta_output = bot_meta.to_output()
    ...
    # 所有しているボットを取得
    owned_bots = find_owned_bots_by_user_id(user.id, limit)
    logger.info(f"所有しているボットの一覧: {owned_bots}")

    # 所有しているボットの処理
    for bot_meta in owned_bots:
        # ボット情報をBotMetaOutputに変換
        bot_meta_output = bot_meta.to_output()

        # オーナーID付きのBotMetaOutputWithOwnerIdに変換
        bot_meta_output_with_owner_id = BotMetaOutputWithOwnerId(**bot_meta_output.model_dump(), owner_user_id=user.id)

        # 戻り値に追加
        bot_metas.append(bot_meta_output_with_owner_id)

    logger.info(f"取得したボットの一覧: {bot_metas}")
    logger.info("end find_all_shared_bots")
    return bot_metas
# 新規追加 ---
...
def get_all_registered_bots(
    ...
    """
    登録されている全てのボット情報を取得する関数
    
    引数:
        user_id (str): ユーザーID
    
    戻り値:
        List[BotMeta]: ボット情報のリスト
    
    処理概要:
        DynamoDBテーブルをスキャンして、全てのボット情報を取得し、
        ユーザーの所有権を判定してBotMetaオブジェクトのリストを返す
    
    例外処理:
        - DynamoDBアクセスエラー
    """
    logger.info("start get_all_registered_bots")
    ...
    MAX_SCAN_COUNT = 5
    scan_params = {
        "FilterExpression": Attr('SK').begins_with("BOT")
    }
    bots =[]
    ...
    while scan_count < MAX_SCAN_COUNT:
        scan_count += 1
        logger.info(f"スキャン回数: {scan_count}")
        
        # スキャンを実行
        try:
            response = table.scan(**scan_params)
        except ClientError as e:
            logger.error(f"DynamoDBスキャン中にエラーが発生しました。: {e}")
            raise

        for item in response["Items"]:
            # start と end の間に last_used_time があれば bots に追加
            last_used_time = item.get("LastUsedTime", 0)
            if start and end:
                if last_used_time < int(start) or last_used_time > int(end):
                    continue
            
            # ユーザーIDと比較して owned の真偽値を設定
            if item["PK"] == user_id:
                owned = True
            else:
                owned = False
            bot_meta = BotMeta.from_dynamo_item(item, owned=owned, is_origin_accessible=True)
            bot_meta_output = bot_meta.to_output()

            # BotMeta型からOwnerUserIdを追加
            bot_meta_output_with_owner_id = BotMetaOutputWithOwnerId(**bot_meta_output.model_dump(), owner_user_id=item["PK"])
            bots.append(bot_meta_output_with_owner_id)
        
        # LastEvaluatedKey があればスキャンを繰り返す
        if "LastEvaluatedKey" not in response:
            break
        scan_params["ExclusiveStartKey"] = response["LastEvaluatedKey"]
    
    logger.info(f"取得したボットの一覧: {bots}")
    logger.info("end get_all_registered_bots")
    return bots
# 新規追加 ---
