...
class BotModel(BaseModel):
    ...
    # 新規追加 ---
    def is_shared_to_user(self, user: User) -> bool:
        """
        ボットがユーザーにアクセス可能かどうかをチェックするメソッド
        
        引数:
            user (User): チェック対象のユーザーオブジェクト
        
        戻り値:
            bool: ユーザーがボットにアクセス可能な場合True、そうでなければFalse
        
        処理概要:
            1. 共有スコープが"all"の場合、全ユーザーにアクセス許可
            2. 許可されたCognitoユーザーリストにユーザーIDが含まれているかチェック
            3. ユーザーが許可されたCognitoグループに属しているかチェック
        """
        # 共有スコープが all の場合は True を返す
        if self.shared_scope == "all":
            return True

        # ユーザーが許可ユーザーに含まれているなら True を返す
        if user.id in self.allowed_cognito_users:
            return True

        # 所属グループの取得
        user_groups = get_user_cognito_groups(user)
        # 所属グループが許可グループに含まれているなら True を返す
        for group in user_groups:
            if group in self.allowed_cognito_groups:
                return True
        
        # どの条件にも当てはまらないなら False を返す
        return False
    # 新規追加 ---
