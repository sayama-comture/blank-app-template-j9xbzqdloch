...
import boto3
from boto3.dynamodb.conditions import Key # 新規追加
from botocore.exceptions import ClientError # 新規追加
...
def handler(event: dict, context: Any) -> None:
    ...
    try:
        ...
        logger.error(f"DynamoDBのクエリ中にエラーが発生しました。: {e}")
        raise

    # 他のユーザーが同じIDのボットを使用している時、削除処理をスキップ
    for item in response["Items"]:
        if item["PK"] != user_id:
            logger.info("コピーされたボットを検知したため、付属リソースの削除をスキップします。")
            return
    # 新規追加 ---
