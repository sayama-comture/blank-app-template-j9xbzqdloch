...
def fetch_all_bots(
    ...
    limit: int | None = None,
    starred: bool = False,
    kind: Literal["private", "mixed", "private,mixed"] = "private",
) -> list[BotMetaOutput | BotMetaOutputWithOwnerId]:
    """Fetch all bots.
    ...
    # kind が private,mixed の場合、所有しているボットと共有されたボットを取得
    elif kind == "private,mixed":
        bots = find_all_shared_bots(user, limit=limit)

    # kind が private,mixed の場合、そのまま返す
    if kind == "private,mixed":
        return bots
